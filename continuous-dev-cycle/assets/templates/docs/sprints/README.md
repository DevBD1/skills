# Sprint Documentation

`docs/sprints/` is the canonical planning and delivery record for this repository. The process is lightweight Scrum: plan enough to make work traceable, keep records concise, and update the sprint as work changes.

For the full idea-to-release flow, see `../development-cycle.md`.

## Cadence

Use 1-week sprints by default unless this repo documents another cadence.

Sprint files use:

```text
docs/sprints/YYYY-MM-DD-sprint-N.md
```

## Backlog

Use `docs/sprints/backlog.md` for candidate work before sprint commitment. Keep items concise but implementation-ready: module/area, user value, acceptance criteria, and validation.

## Sprint Planning

At sprint start, define sprint goal, dates, assumptions, committed items, acceptance criteria, release target, changelog impact, and validation expectations.

## Work Log

Record meaningful updates: status changes, blockers, scope changes, decisions, and verification results. Do not log every small edit.

## Review And Retro

At completion, summarize completed work, validation, changelog status, incomplete items, what to keep, what to change, and carryover.
