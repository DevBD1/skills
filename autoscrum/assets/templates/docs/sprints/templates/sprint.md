# Sprint N: YYYY-MM-DD

## Sprint Goal

State the single most important outcome for the sprint.

## Product Alignment

- Link the sprint goal to the repo's product, roadmap, or maintenance priorities.

## Dates

- **Start:** YYYY-MM-DD
- **End:** YYYY-MM-DD
- **Cadence:** 1 week

## Capacity And Assumptions

- Note capacity, constraints, risks, and external dependencies.

## Committed Items

### Item 1: Short Title

- [ ] **Status:** Not started
- **Module/area:** `<module-or-package>`
- **User value:** Explain why this work matters.
- **Release target:** `docs/releases/vX.Y.Z.md` or `Not release-relevant`.
- **Changelog impact:** `Added`, `Changed`, `Fixed`, `Deprecated`, `Removed`, `Security`, or `None`.
- **Acceptance criteria:**
  - [ ] Observable behavior or documentation outcome.
  - [ ] Edge case, compatibility, or config expectation.
- **Validation:**
  - [ ] `<validation-command>`

## Work Log

Dated bullets per item; log status changes, blockers, scope changes, decisions, and verification results — not every edit.

- YYYY-MM-DD — Item 1: Initial note.

## Review

- **Completed items:** List completed items and shipped artifacts.
- **Verification:** Summarize commands and manual checks.
- **Changelog status:** Note whether `CHANGELOG.md` was updated or why not.
- **Release channel:** Note tag, artifact, deploy, package publish, external changelog, or why none applies.
- **Not completed:** List incomplete items and why.

## Retro

- **Keep:** What worked and should continue.
- **Change:** What should change next sprint.
- **Follow up:** Backlog candidates or carryover notes.

## Carryover

- [ ] Item or follow-up to move into the next sprint or backlog.
